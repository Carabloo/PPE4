	...
	static function getClientHeaders() {
		if(php_Web::$_client_headers === null) {
			...
			$h = php_Lib::hashOfAssociativeArray($_SERVER);
			{
				...
				while($k->hasNext()) {
					$k1 = $k->next();
					if(_hx_substr($k1, 0, 5) === "HTTP_") {
						$tmp = php_Web::$_client_headers;
						$tmp1 = _hx_substr($k1, 5, null);
						$tmp->add(_hx_anonymous(array("header" => $tmp1, "value" => $h->get($k1))));
						unset($tmp1,$tmp);
					} else {
						if(_hx_substr($k1, 0, 8) === "CONTENT_") {
							$tmp2 = php_Web::$_client_headers;
							$tmp2->add(_hx_anonymous(array("header" => $k1, "value" => $h->get($k1))));
							unset($tmp2);
						}
					}
					unset($k1);
			...
			if(isset($_SERVER['REDIRECT_HTTP_AUTHORIZATION'])) {
				$tmp3 = php_Web::$_client_headers;
				$tmp3->add(_hx_anonymous(array("header" => "AUTHORIZATION", "value" => (string)$_SERVER['REDIRECT_HTTP_AUTHORIZATION'])));
			} else {
				if(isset($_SERVER['PHP_AUTH_USER'])) {
					$basic_pass = isset($_SERVER['PHP_AUTH_PW']) ? (string)$_SERVER['PHP_AUTH_PW'] : '';
					$tmp4 = php_Web::$_client_headers;
					$tmp4->add(_hx_anonymous(array("header" => "AUTHORIZATION", "value" => "Basic " . _hx_string_or_null(base64_encode($_SERVER['PHP_AUTH_USER'] + ':' + $basic_pass)))));
				} else {
					if(isset($_SERVER['PHP_AUTH_DIGEST'])) {
						$tmp5 = php_Web::$_client_headers;
						$tmp5->add(_hx_anonymous(array("header" => "AUTHORIZATION", "value" => (string)$_SERVER['PHP_AUTH_DIGEST'])));
					}
				}
			}
		}
