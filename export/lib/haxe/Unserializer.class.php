...
class haxe_Unserializer {
	...
	public function unserialize() {
		...
		case 115:{
			...
			$max = $i1 + ($len - $rest);
			$bytes = haxe_io_Bytes::alloc(($len >> 2) * 3 + $size);
			$bpos = 0;
			while($i1 < $max) {
				$i1 = $i1 + 1;
				$c11 = $codes[ord(substr($buf,$i1 - 1,1))];
				$i1 = $i1 + 1;
				$c2 = $codes[ord(substr($buf,$i1 - 1,1))];
				{
					$bpos = $bpos + 1;
					$pos = $bpos - 1;
					{
				...
				$i1 = $i1 + 1;
				$c3 = $codes[ord(substr($buf,$i1 - 1,1))];
				{
					$bpos = $bpos + 1;
					$pos1 = $bpos - 1;
					{
				...
				$i1 = $i1 + 1;
				$c4 = $codes[ord(substr($buf,$i1 - 1,1))];
				{
					$bpos = $bpos + 1;
					$pos2 = $bpos - 1;
					{
			...
			if($rest >= 2) {
				$i1 = $i1 + 1;
				$c12 = $codes[ord(substr($buf,$i1 - 1,1))];
				$i1 = $i1 + 1;
				$c21 = $codes[ord(substr($buf,$i1 - 1,1))];
				{
					$bpos = $bpos + 1;
					$pos3 = $bpos - 1;
					{
				...
				if($rest === 3) {
					$i1 = $i1 + 1;
					$c31 = $codes[ord(substr($buf,$i1 - 1,1))];
					{
						$bpos = $bpos + 1;
						$pos4 = $bpos - 1;
						{
