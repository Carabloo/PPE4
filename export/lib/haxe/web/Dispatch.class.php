...
class haxe_web_Dispatch {
	...
	public function match($v, $r, $opt) {
		...
		case 5:{
			...
			$en = Type::resolveEnum(_hx_deref($r)->params[0]);
			if($en === null) {
		...
		case 7:{
			...
			$cl = Type::resolveClass(_hx_deref($r)->params[0]);
			if($cl === null) {
				throw new HException("assert");
			}
			$o = $cl->manager->unsafeGet($v3, _hx_deref($r)->params[1]);
			if($o === null) {
				throw new HException(haxe_web_DispatchError::$DEInvalidValue);
			}
			return $o;
		...
		case 8:{
			...
			return $this->match($v, _hx_deref($r)->params[0], true);
		}break;
		}
	}
	...
	public function loop($args, $r) {
		switch($r->index) {
		case 0:{
			$args->push($this->match($this->parts->shift(), _hx_deref($r)->params[0], false));
		}break;
		case 1:{
		...
		case 2:{
			$this->loop($args, _hx_deref($r)->params[0]);
			$args->push($this->checkParams(_hx_deref($r)->params[1], _hx_deref($r)->params[2]));
		}break;
		case 3:{
			$this->loop($args, _hx_deref($r)->params[0]);
	...
	static function extractConfig($obj) {
		$dc = haxe_rtti_Meta::getType(Type::getClass($obj));
