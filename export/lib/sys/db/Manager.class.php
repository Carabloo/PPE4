...
class sys_db_Manager {
					...
					case 8:{
						if(!Std::is($val, _hx_qtype("Bool"))) {
							if(Std::is($val, _hx_qtype("Int"))) {
								...
								if(Std::is($val, _hx_qtype("String"))) {
									switch(_hx_string_call($val, "toLowerCase", array())) {
									case "1":case "true":{
										$val = true;
									}break;
									case "0":case "false":{
	...
	public function makeCacheKey($x) {
		if($this->table_keys->length === 1) {
			...
			$tmp = Std::string($k);
			return _hx_string_or_null($tmp) . _hx_string_or_null($this->table_name);
		}
		$s = new StringBuf();
		{
			$_g = 0;
	...
	static function nullCompare($a, $b, $eq) {
		...
		if($tmp) {
			if($eq) {
				return "" . _hx_string_or_null($b) . " IS NULL";
			} else {
				return "" . _hx_string_or_null($b) . " IS NOT NULL";
			}
		} else {
			$tmp1 = null;
			if($b !== null) {
			...
			if($tmp1) {
				if($eq) {
					return "" . _hx_string_or_null($a) . " IS NULL";
				} else {
					return "" . _hx_string_or_null($a) . " IS NOT NULL";
				}
			}
		}
		if(sys_db_Manager::$cnx->dbName() !== "MySQL") {
			...
			return _hx_string_or_null($a) . _hx_string_or_null($tmp2) . _hx_string_or_null($b);
		}
